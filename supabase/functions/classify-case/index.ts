...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: classificationId, error: applyError } = await serviceClient.rpc("apply_case_classification", {
      p_case_id: caseId,
    ...
    if (applyError) {
      return jsonResponse({ error: "Failed to apply case classification", detail: applyError.message }, 500);
    }
