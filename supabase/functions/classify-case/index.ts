...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const requestedPolicyVersionId = body?.policy_version_id ?? null;
    ...
    let policyVersionId = requestedPolicyVersionId;

    if (!policyVersionId) {
      const { data: policyId, error: policyRpcError } = await serviceClient.rpc(
        "get_published_policy_version_id",
      );

      if (policyRpcError) {
        return jsonResponse(
          { error: "Failed to resolve published policy", detail: policyRpcError.message },
          500,
        );
      }

      policyVersionId = policyId ?? null;
    }
    ...
    const { data: restrictionHits, error: hitsError } = await serviceClient
      ...
      .select("id, case_id, document_id, policy_version_id, rule_id, severity, action, matched_text, excerpt, page_number, details, created_at")
      .eq("case_id", caseId)
      .order("created_at", { ascending: true });

    if (hitsError) {
      return jsonResponse(
        { error: "Failed to load case_restriction_hits", detail: hitsError.message },
        500,
      );
    }
    ...
    const highFailChecks = failChecks.filter(
      (row) => (row.severity ?? "").toLowerCase() === "high",
    );

    const autoRejectHits = hitRows.filter(
      (row) => (row.action ?? "").toUpperCase() === "AUTO_REJECT",
    );
    const manualLegalHits = hitRows.filter(
      (row) => (row.action ?? "").toUpperCase() === "FLAG_MANUAL_LEGAL",
    );
    const allowButYellowHits = hitRows.filter(
      (row) => (row.action ?? "").toUpperCase() === "ALLOW_BUT_YELLOW",
    );
    const confirmedHits = hitRows.filter(
      (row) => (row.severity ?? "").toUpperCase() === "CONFIRMED",
    );
    ...
    const { data: classificationId, error: applyError } = await serviceClient.rpc(
      "apply_case_classification",
      {
        p_case_id: caseId,
        p_classification: classification,
        p_reason_summary: reasonSummary,
    ...
    if (applyError) {
      return jsonResponse(
        { error: "Failed to apply case classification", detail: applyError.message },
        500,
      );
    }
