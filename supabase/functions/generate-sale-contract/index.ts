...
async function convertHtmlToPdf(html: string, pdfShiftKey: string): Promise<Uint8Array | null> {
  try {
    const response = await fetch("https://api.pdfshift.io/v3/convert/pdf", {
      ...
      body: JSON.stringify({ source: html, landscape: false, use_print: true, margin: { top: "20mm", bottom: "20mm", left: "20mm", right: "20mm" } }),
    });
    if (!response.ok) { console.error("PDFShift error:", response.status, await response.text()); return null; }
    return new Uint8Array(await response.arrayBuffer());
  } catch (err) { console.error("PDFShift conversion error:", err); return null; }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Hívó azonosítása: service role VAGY admin felhasználó
    // Service role token (pl. submit-case belső hívás) = elfogadott
    // Admin JWT = elfogadott (admin ellenőrzéssel)
    const isServiceRoleCall = token === serviceRoleKey;
    let callerUserId: string | null = null;

    if (!isServiceRoleCall) {
      // Normál admin JWT hívás — ellenőrizzük, hogy admin-e
    ...
    const { data: sellerProfile } = await serviceClient.from("profiles").select("full_name, email, phone").eq("id", caseData.seller_user_id).single();
    const { data: sellerDetails } = await serviceClient.from("seller_profiles").select("id_number, tax_id, billing_name, billing_address, birth_date, birth_place, birth_name, mother_name").eq("user_id", caseData.seller_user_id).maybeSingle();
    const { data: weekOffer } = await serviceClient.from("week_offers").select("*").eq("case_id", case_id).maybeSingle();
    const { data: abbaziaShares } = await serviceClient.from("abbazia_shares").select("*").eq("case_id", case_id).maybeSingle();

    let resortName = weekOffer?.resort_name_raw ?? "—";
    if (weekOffer?.resort_id) {
      const { data: resort } = await serviceClient.from("resorts").select("name").eq("id", weekOffer.resort_id).maybeSingle();
      if (resort?.name) resortName = resort.name;
    }

    // Policy settings (céges adatok)
    let policySettings: Record<string, string> = {};
    const settingKeys = ["buyer_name", "buyer_address", "buyer_company_number", "buyer_tax_number", "buyer_representative", "contact_email"];
    const { data: policyRows } = await serviceClient.from("policy_settings").select("setting_key, setting_value").eq("policy_version_id", caseData.policy_version_id_applied ?? "").in("setting_key", settingKeys);

    if (!policyRows || policyRows.length === 0) {
      const { data: publishedPolicy } = await serviceClient.from("policy_versions").select("id").eq("status", "published").order("published_at", { ascending: false }).limit(1).maybeSingle();
      if (publishedPolicy) {
        const { data: fallbackRows } = await serviceClient.from("policy_settings").select("setting_key, setting_value").eq("policy_version_id", publishedPolicy.id).in("setting_key", settingKeys);
    ...
    for (const contractType of contractTypes) {
      ...
      const { error: uploadErr } = await serviceClient.storage.from(bucket).upload(storagePath, fileContent, { contentType, upsert: true });
      if (uploadErr) {
      ...
      const { data: existing } = await serviceClient.from("contracts").select("id").eq("case_id", case_id).eq("contract_type", contractType).maybeSingle();
      if (existing) {
        await serviceClient.from("contracts").update({ status: "generated", generated_storage_bucket: bucket, generated_storage_path: storagePath, generated_file_name: fileName, generated_at: now }).eq("id", existing.id);
      } else {
        await serviceClient.from("contracts").insert({ case_id, contract_type: contractType, status: "generated", generated_storage_bucket: bucket, generated_storage_path: storagePath, generated_file_name: fileName, generated_at: now });
      }

      generatedContracts.push({ type: contractType, path: storagePath });
    }

    await serviceClient.from("cases").update({ status: "contract_generated", updated_at: new Date().toISOString() }).eq("id", case_id);

    await serviceClient.from("audit_logs").insert({
      entity_type: "cases",
      entity_id: case_id,
      action: "contracts_generated",
      performed_by_user_id: callerUserId,  // null ha service role hívás
      source: "edge_function",
      new_data: { is_abbazia: isAbbazia, contract_count: contractTypes.length, contracts: generatedContracts, used_db_templates: Object.keys(templateMap), triggered_by: isServiceRoleCall ? "auto_green_submit" : "admin_manual" },
    });

    return jsonResponse({ success: true, is_abbazia: isAbbazia, contract_count: contractTypes.length, contracts: generatedContracts });

  } catch (err) {
    console.error("generate-sale-contract unhandled error:", err);
    return jsonResponse({ error: "Internal server error", detail: err instanceof Error ? err.message : "Unknown error" }, 500, req);
  }
});
