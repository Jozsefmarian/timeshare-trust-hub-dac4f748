...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (caseError) {
      return jsonResponse(
        { error: "Failed to load case", detail: caseError.message },
        500,
      );
    }
    ...
    if (!allowedSourceStatuses.includes(caseRow.status)) {
      return jsonResponse(
        {
          error: "Case cannot be submitted from current status",
    ...
    // 2) Minimál precheck: seller profile legyen
    if (!caseRow.seller_profile_id) {
      return jsonResponse(
        {
          error: "Missing seller profile",
          detail: "Seller profile must exist before submit",
        },
        400,
      );
    }

    // 3) Minimál precheck: week offer legyen
    ...
    if (weekOfferError) {
      return jsonResponse(
        { error: "Failed to load week offer", detail: weekOfferError.message },
        500,
      );
    }
    ...
    if (!weekOffer) {
      return jsonResponse(
        {
          error: "Missing week offer",
          detail: "Week offer must exist before submit",
        },
        400,
      );
    }

    // 4) Minimál precheck: legyen legalább 1 ténylegesen feltöltött doksi
    ...
    if (docsError) {
      return jsonResponse(
        { error: "Failed to load documents", detail: docsError.message },
        500,
      );
    }
    ...
    if (!uploadedDocs || uploadedDocs.length === 0) {
      return jsonResponse(
        {
          error: "Missing uploaded documents",
          detail: "At least one uploaded document is required before submit",
        },
        400,
      );
    }

    // 5) AI pipeline legyen kész, és classification meglegyen
    if (caseRow.ai_pipeline_status !== "completed") {
      return jsonResponse(
        {
          error: "AI pipeline not completed",
          detail: `Current ai_pipeline_status: ${caseRow.ai_pipeline_status ?? "null"}`,
        },
        409,
      );
    }
      ...
      return jsonResponse(
        {
          ...
          detail: "Case classification must be green, yellow or red before submit",
        },
        409,
      );
    }
    ...
    // 6) Végső submit update service klienssel
    ...
    if (updateError) {
      return jsonResponse(
        { error: "Failed to submit case", detail: updateError.message },
        500,
      );
    }

    // 7) Audit log opcionális, de hasznos
    ...
    if (auditError) {
      console.error("submit-case audit log insert failed:", auditError);
    }
