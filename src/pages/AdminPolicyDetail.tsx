...
import {
  ArrowLeft, Save, Globe, Archive, Plus, Trash2, ShieldCheck, AlertTriangle, Tag,
} from "lucide-react";
...
const mockRestrictions: RestrictionRule[] = [
  { id: "r1", name: "Csőd kulcsszó", type: "Kulcsszó tiltás", field: "megjegyzés", operator: "tartalmazza", value: "csőd, felszámolás", severity: "Magas", message: "Csődeljáráshoz kapcsolódó ügy nem fogadható el." },
  { id: "r2", name: "Tiltott üdülőhely", type: "Üdülőhely tiltás", field: "üdülőhely", operator: "egyenlő", value: "Teszt Resort", severity: "Magas", message: "Ez az üdülőhely nem elfogadható." },
  { id: "r3", name: "Lejárt jog", type: "Lejárati év korlátozás", field: "jogosultság_vége", operator: "kisebb mint", value: "2025", severity: "Közepes", message: "Lejárt jogosultság esetén egyedi elbírálás szükséges." },
];

const mockClassifications: ClassificationRule[] = [
  { id: "c1", name: "Tiszta ügy", condition: "Nincs korlátozó szabály találat és minden dokumentum elfogadva", result: "Zöld" },
  { id: "c2", name: "Részleges probléma", condition: "Közepes súlyosságú szabály találat vagy hiányzó dokumentum", result: "Sárga" },
...
export default function AdminPolicyDetail() {
  ...
  const [description, setDescription] = useState(isNew ? "" : "Az alapértelmezett szabályrendszer, amely az összes beérkező ügyre vonatkozik.");
  ...
  const addRestriction = () => {
    setRestrictions([...restrictions, {
      id: `r${Date.now()}`, name: "", type: ruleTypes[0], field: "", operator: operators[0], value: "", severity: severities[0], message: "",
    }]);
  };
  ...
  const statusBadge = status === "Publikált"
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline"><Archive className="h-4 w-4 mr-2" />Archiválás</Button>
            <Button variant="outline"><Globe className="h-4 w-4 mr-2" />Publikálás</Button>
            <Button><Save className="h-4 w-4 mr-2" />Mentés</Button>
          </div>
        </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="pl. Alapértelmezett szabályrendszer" />
                  </div>
                  <div className="space-y-2">
                    <Label>Verzió</Label>
                ...
                <div className="space-y-2">
                  <Label>Leírás</Label>
                  <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Szabályrendszer rövid leírása..." rows={3} />
                </div>
              </CardContent>
            </Card>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="outline" size="sm" onClick={addRestriction}>
                    <Plus className="h-4 w-4 mr-1" />Új szabály
                  </Button>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                {restrictions.length === 0 && (
                  <p className="text-muted-foreground text-sm text-center py-8">Nincs korlátozó szabály. Kattintson az "Új szabály" gombra.</p>
                )}
                  ...
                  <Card key={rule.id} className="border-border/50">
                    <CardContent className="pt-4 space-y-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
                          <div className="space-y-1.5">
                            ...
                            <Input value={rule.name} onChange={(e) => updateRestriction(rule.id, "name", e.target.value)} placeholder="Szabály neve" />
                          </div>
                          <div className="space-y-1.5">
                            ...
                            <Select value={rule.type} onValueChange={(v) => updateRestriction(rule.id, "type", v)}>
                              ...
                              <SelectContent>{ruleTypes.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                            </Select>
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-xs">Mező neve</Label>
                            <Input value={rule.field} onChange={(e) => updateRestriction(rule.id, "field", e.target.value)} placeholder="pl. megjegyzés" />
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-xs">Operátor</Label>
                            <Select value={rule.operator} onValueChange={(v) => updateRestriction(rule.id, "operator", v)}>
                              <SelectTrigger><SelectValue /></SelectTrigger>
                          ...
                          <div className="space-y-1.5">
                            ...
                            <Input value={rule.value} onChange={(e) => updateRestriction(rule.id, "value", e.target.value)} placeholder="Érték" />
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-xs">Súlyosság</Label>
                            <Select value={rule.severity} onValueChange={(v) => updateRestriction(rule.id, "severity", v)}>
                              <SelectTrigger><SelectValue /></SelectTrigger>
                              <SelectContent>{severities.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                            </Select>
                          </div>
                        </div>
                        <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-destructive shrink-0 mt-5" onClick={() => removeRestriction(rule.id)}>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Textarea value={rule.message} onChange={(e) => updateRestriction(rule.id, "message", e.target.value)} placeholder="Hibaüzenet szövege..." rows={2} />
                      </div>
                    </CardContent>
                  </Card>
                ))}
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="outline" size="sm" onClick={addClassification}>
                    <Plus className="h-4 w-4 mr-1" />Új minősítési szabály
                  </Button>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                    ...
                    <CardContent className="pt-4 space-y-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
                          <div className="space-y-1.5">
                            <Label className="text-xs">Szabály neve</Label>
                            <Input value={rule.name} onChange={(e) => updateClassification(rule.id, "name", e.target.value)} placeholder="Szabály neve" />
                          </div>
                          <div className="space-y-1.5">
                            <Label className="text-xs">Eredmény minősítés</Label>
                            <Select value={rule.result} onValueChange={(v) => updateClassification(rule.id, "result", v)}>
                              <SelectTrigger><SelectValue /></SelectTrigger>
                              <SelectContent>{classifications.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                            </Select>
                          </div>
                        </div>
                        <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-destructive shrink-0 mt-5" onClick={() => removeClassification(rule.id)}>
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Textarea value={rule.condition} onChange={(e) => updateClassification(rule.id, "condition", e.target.value)} placeholder="Milyen feltétel esetén érvényes ez a minősítés..." rows={2} />
                      </div>
                    </CardContent>
                  </Card>
                ))}
              ...
              <CardContent className="pt-4">
                <div className="flex gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">A szabályrendszer módosítása után a „Publikálás" gombbal teheti aktívvá a változtatásokat.</p>
                  </div>
                </div>
              </CardContent>
            </Card>
